...
function projects_function( $attrs ) {
	...
	if ( $the_query->have_posts() ) {
		...
		$str .= '<ul id="agent-list" class="service-grid">';
		while ( $the_query->have_posts() ) {
			$the_query->the_post();

			$str .= '<li id="' . get_the_ID() . '" class="agent_block ' . get_post_meta( get_the_ID(), 'project_info_project_type', true ) . '" style="margin-left:0px; margin-right: 10px; margin-bottom: 10px">';
